...
def Kitaev2D_local_energies(J1, J2, J3,  Nx, Ny, samples, queue_samples, log_ampl_tensor, samples_placeholder, log_amplitudes, entropy_lambda, batch_size, sess):
    ...
    for i in range(Ny): 
        ...
        #even row, interction on odd spins with previous row
            values = 1 - 2 * ( (samples[:,i-1,1::2] + samples[:,i,1::2])%2 )
        else:
        #odd row, interction on even spins with previous row
            values = 1 - 2 * ( (samples[:,i-1,0::2] + samples[:,i,0::2])%2 )
       
        #sum over axis 1 = sum over the interactions on the same row
        #accumulate in same array = sum over rows
        local_energies += np.sum(values*J3, axis=1)
               
    queue_samples[0] = samples #storing the diagonal samples

    #Non-diagonal elements
    y_states_sign_map = []
    Ns = Nx-1 #Number of new samples per row
    Ns_parity = Ns%2==0
    if J1 != 0 or J2 !=0:
        #Loop over rows
        for i in range(Ny):
            if i%2 == 0:
                ...
                for j in range(Nx//2):
                    ...
                    x_interaction_states[:, i, 2*j]  = 1 - samples[:, i, 2*j]
                    x_interaction_states[:, i, 2*j+1]= 1 - samples[:, i, 2*j+1]

                    #Always remember x-interactions in odd index
                        #works because i is even 
                    queue_samples[Ns * i + 2*j +1] = x_interaction_states 
                    
                    #yy on spin 2j+1 and 2j+2
                    y_interaction_states = np.copy(samples)
                    y_interaction_states[:, i, 2*j+1] = 1 - samples[:, i, 2*j+1]
                    y_interaction_states[:, i, 2*(j+1)] = 1 - samples[:, i, 2*(j+1)]
                    
                    #Remember the sign of each interaction. It's -1 if the same spin
                    # 1 - 2*[T,F] --> [-1, +1]
                    sign_map = 1 - 2 *  ( (y_interaction_states[:, i, 2*j+1] == y_interaction_states[:, i, 2*(j+1)])%2 )
                    y_states_sign_map.append(sign_map)

                    #Always remember y-interactions in even index
                    queue_samples[Ns * i + 2*j+2] = y_interaction_states
            else:
                 #on odd rows interaction is yy--xx--yy...
                for j in range(Nx//2): 
                    #xx on spin 2j and 2j+1
                    y_interaction_states=np.copy(samples)
                    y_interaction_states[:, i, 2*j]  = 1 - samples[:, i, 2*j]
                    y_interaction_states[:, i, 2*j+1]= 1 - samples[:, i, 2*j+1]
                    ...
                    #Always remember y-interactions in even index
                    #and x-interaction in odd index
                        #because i is odd depends on Ns
                    if Ns_parity:
                        queue_samples[Ns * i + 2*j +1] = x_interaction_states 
                        queue_samples[Ns * i + 2*j +2] = y_interaction_states
                    else:
                        queue_samples[Ns * i + 2*j +1] = y_interaction_states 
                        queue_samples[Ns * i + 2*j +2] = x_interaction_states
...
def run_2DKitaev(numsteps = 2*10**4, systemsize_x = 5, systemsize_y = 5, 
    ...
    wf=RNNwavefunction(Nx,Ny,hilbert_dim= input_dim, units=units,cell=MDRNNcell,seed = seed) #contains the graph with the RNNs
...
#        print('The sum of params is {0}'.format(sum))
    ...
    with tf.compat.v1.variable_scope(wf.scope,reuse=tf.compat.v1.AUTO_REUSE):
        with wf.graph.as_default():
            ...
            Ns = Nx -1 #new samples per row after interaction
