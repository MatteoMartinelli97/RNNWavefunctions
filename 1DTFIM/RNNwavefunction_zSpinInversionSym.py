...
class SpinFlipSymRNNwavefunction(object):
    ...
    def sample(self,numsamples,inputdim):
        ...
        with self.graph.as_default(): #Call the default graph, used if not willing to create multiple graphs.
            samples = []
            with tf.variable_scope(self.scope,reuse=tf.AUTO_REUSE):
                b=np.zeros((int(0.5*numsamples),inputdim)).astype(np.float64)
                b[:,0]=np.zeros(int(0.5*numsamples))
                #b = state of sigma_0 for half of the samples --> the other half will be sampled according to the symmmetry

                inputs=tf.constant(dtype=tf.float32,value=b,shape=[int(0.5*numsamples),inputdim]) #Feed the table b in tf.
                ...
                rnn_state=self.rnn.zero_state(int(0.5*numsamples),dtype=tf.float32)
                #zero state returns a zero filled tensor withs shape = (numsamples/2, num_units)

                for n in range(self.N):
                    rnn_output, rnn_state = self.rnn(inputs, rnn_state)
                    output=self.dense(rnn_output)
                    sample_temp=tf.reshape(tf.multinomial(tf.log(output),num_samples=1, output_dtype=tf.int32),[-1,])
                    symm_temp = tf.ones(sample_temp.shape[0], dtype=tf.int32) - sample_temp
                    samples.append(tf.concat(sample_temp, symm_temp))
                    inputs=tf.one_hot(sample_temp,depth=self.outputdim)

        self.samples = tf.stack(values= samples ,axis=1) # (self.N, numsamples) to (numsamples, self.N): Generate self.numsamples vectors of size self.N spin containing 0 or 1
        return self.samples

    def log_probability(self,samples,inputdim):
        """
            calculate the log-probabilities of ```samples`` with a spin-flip symmetry
            ------------------------------------------------------------------------
            Parameters:
        ...
        with self.graph.as_default():
            ...
            #Flip all the spin in every configuration to impose the symmetry---------------------------
            samples_rev = 1-samples
            ...
            return tf.log(0.5*(tf.exp(log_probs1)+tf.exp(log_probs2))) #We take the average of P1 and P2 to impose the spin flip symmetry
